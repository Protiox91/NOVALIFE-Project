		...
		if data.current.value == 'vehicle_list' then
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = 'Vehicule Urgence', value = 'contender'}
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = 'Prendre Intervention', value = 'serv_moldu'},
			{label = 'Stopper Intervention', value = 'serv_moldu2'}
		}
	}, function(data, menu)
	...
	elseif data.current.value == 'serv_moldu' then
		StartNPCJob()
			

	elseif data.current.value == 'serv_moldu2' then
		StopNPCJob()
...
RegisterNetEvent('Brook:menumecano')
AddEventHandler('Brook:menumecano', function()
	OpenMobileMechanicActionsMenu()
end)
