...
RegisterCommand("jail", function(src, args, raw)
	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "JUDGE",  Firstname .. " " .. Lastname .. " est maintenant emprisonné pour : " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
			...
			TriggerClientEvent("esx:showNotification", src, "Cette personne n'est pas en ville !")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Tu n'est pas de la police !")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "JUDGE",  Firstname .. " " .. Lastname .. " Est maintenant emprisonné pour : " .. jailReason }, color = { 249, 166, 0 } })
	end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Merci ! Tiens un peu d'argent.")
end)
