...
function JailLogin()
	...
	ESX.ShowNotification("Tu t'est échappé la dernière fois, tu retournes en prison !")
...
function UnJail()
	...
	ESX.ShowNotification("Tu est maintenant libre, soit sage ! Bonne chance !")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Il te reste " .. jailTime .. " minutes en prison !")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Prépare le packet"
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Tu as déjà pris ce packet !")
							end
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "En cours de Packaging... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Poser le packet", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Emprisonner une personne", value = "jail_closest_player" },
				{ label = "Libérer une personne", value = "unjail_player" }
			}
		}, 
	function(data, menu)
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Personne n'est à coté de toi !")
					else
						ESX.UI.Menu.Open(
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Prisonnier : " .. playerArray[i].name .. " | Temps : " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
				end
