...
function Teleport(entity, coords, cb)
		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				elements	= {
					...
					{label = 'Prendre une intervention', value = 'npc_job'},
					{label = 'Annuler l\'intervention', value = 'npc_job2'}
				}
			}, function(data, menu)
				...
				if closestPlayer == -1 or closestDistance > 1.0 then
					...
					elseif data.current.value == 'npc_job' then
						StartNPCJOB()
					elseif data.current.value == 'npc_job2' then
						StopAmbulanceJob()
					end					
...
function FastTravel(coords, heading)
	...
	Teleport(playerPed, coords, function()
		DoScreenFadeIn(800)
...
Citizen.CreateThread(function()
	while true do
		...
		elseif ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' and not IsDead then
			if IsControlJustReleased(0, Keys['F6']) and (GetGameTimer() - GUI.Time) > 150 then
			...
			if #authorizedVehicles > 0 then
				for k,vehicle in ipairs(authorizedVehicles) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(vehicle.label, _U('shop_item', GroupDigits(vehicle.price))),
						name  = vehicle.label,
						model = vehicle.model,
						price = vehicle.price,
						type  = 'car'
		...
		for k,v in ipairs(vehicles) do
			...
			if GetVehicleNumberOfPassengers(v) == 0 and IsVehicleSeatFree(v, -1) then
				table.insert(vehiclePlates, {
					vehicle = v,
					plate = Trim(GetVehicleNumberPlateText(v))
				})
			end
		end
	else
	...
	ESX.TriggerServerCallback('esx_ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			...
			while DoesEntityExist(vehicleId.vehicle) do
				...
				if #vehicles > 0 then
					for k,v in ipairs(vehicles) do
						if Trim(GetVehicleNumberPlateText(v)) == vehicleId.plate then
		...
		if data.current.action == 'buy_helicopter' then
			...
			if #authorizedHelicopters > 0 then
				for k,helicopter in ipairs(authorizedHelicopters) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(helicopter.label, _U('shop_item', GroupDigits(helicopter.price))),
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm',
		{
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_ambulancejob:buyJobVehicle', function (bought)
					if bought then
						ESX.ShowNotification(_U('vehicleshop_bought', data.current.name, GroupDigits(data.current.price)))
						...
						Teleport(playerPed, restoreCoords)
					else
...
RegisterNetEvent("Brook:menuamb")
AddEventHandler("Brook:menuamb", function()
	OpenMobileAmbulanceActionsMenu()
end)
