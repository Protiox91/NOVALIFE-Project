  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_hitmanjob:giveWeapon', weapon,  1000)
      end,
      function(data, menu)
...
function OpenVehicleSpawnerMenu(station, partNum)
  ...
  local vehicles = Config.HitmanStations[station].Vehicles
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    for i=1, #Config.HitmanStations[station].AuthorizedVehicles, 1 do
      local vehicle = Config.HitmanStations[station].AuthorizedVehicles[i]
...
function OpenHitmanActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'hitman_actions',
    {
      title    = 'Hitman',
      align    = 'top-left',
      elements = {
    ...
    function(data, menu)
      ...
      if data.current.value == 'citizen_interaction' then
		...
		{label = "Player Finder",            value = 'find'}	
            },
          },
          function(data2, menu2)
            ...
            if distance ~= -1 and distance <= 3.0 then
              ...
              if data2.current.value == 'handcuff' then
                TriggerServerEvent('esx_hitmanjob:handcuff', GetPlayerServerId(player))
              end

              if data2.current.value == 'drag' then
                TriggerServerEvent('esx_hitmanjob:drag', GetPlayerServerId(player))
              end

              if data2.current.value == 'put_in_vehicle' then
                TriggerServerEvent('esx_hitmanjob:putInVehicle', GetPlayerServerId(player))
              end

              if data2.current.value == 'out_the_vehicle' then
                  TriggerServerEvent('esx_hitmanjob:OutVehicle', GetPlayerServerId(player))
              end

              if data2.current.value == 'fine' then
                OpenFineMenu(player)
              end
		if data2.current.value == 'find' then
                OpenFindMenu()
              end
...
function GetPlayers()
    ...
    for i = 0, 31 do
        if NetworkIsPlayerActive(i) then
           table.insert(players, {label = GetPlayerServerId(i) .. " | " ..GetPlayerName(i), value = GetPlayerServerId(i)})
       end
   end
...
local elements=GetPlayers()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'find',
    {
      title    = "Choose Player",
      align    = 'top-left',
      elements = elements
...
pos = GetEntityCoords(GetPlayerPed(data.current.value))
x = pos.x
y = pos.y
z = pos.z
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          TriggerServerEvent('esx_hitmanjob:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
  ...
  ESX.TriggerServerCallback('esx_hitmanjob:getFineList', function(fines)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_hitman', _U('fine_total') .. label, amount)
        else
...
function OpenVehicleInfosMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_hitmanjob:getVehicleInfos', function(infos)
  ...
  ESX.TriggerServerCallback('esx_hitmanjob:getArmoryWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_hitmanjob:buy', function(hasEnoughMoney)
...
AddEventHandler('esx_hitmanjob:hasEnteredMarker', function(station, part, partNum)
  ...
  if part == 'HelicopterSpawner' then
    ...
    local helicopters = Config.HitmanStations[station].Helicopters
...
AddEventHandler('esx_hitmanjob:hasEnteredEntityZone', function(entity)
  ...
  if PlayerData.job ~= nil and PlayerData.job.name == 'hitman' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_hitmanjob:drag', function(cop)
...
AddEventHandler('esx_hitmanjob:OutVehicle', function(t)
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'hitman' then
      ...
      for k,v in pairs(Config.HitmanStations) do
        ...
        if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'hitman' and PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'hitman' and (GetGameTimer() - GUI.Time) > 150 then
   ...
   if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'hitman' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'hitman_actions') and (GetGameTimer() - GUI.Time) > 150 then
...
RegisterNetEvent('NB:openMenuHitman')
AddEventHandler('NB:openMenuHitman', function()
	OpenHitmanActionsMenu()
end)
