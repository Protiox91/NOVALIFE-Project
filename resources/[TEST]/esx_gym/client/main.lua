...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuie sur ~INPUT_CONTEXT~ pour louer un véhicule')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Vous avez déjà un véhicule")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuie sur ~INPUT_CONTEXT~ pour ouvrir le menu de ~b~gym~w~')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuie sur ~INPUT_CONTEXT~ pour entrainer ~g~vos bras')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("~g~Echauffement~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Repose-toi ~r~60 secondes ~w~!")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Vous devez avoir un abonnement pour ~r~s'entrainer")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuie sur ~INPUT_CONTEXT~ pour faire des ~g~pull-ups')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuie sur ~INPUT_CONTEXT~ pour faire des ~g~pushups')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuie sur ~INPUT_CONTEXT~ pour faire du ~g~yoga')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Vous pouvez vous y remettre...")
	end
end
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("On est ouvert ~g~24h/h~w~. Bienvenue !")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Shake Protein ($6)', value = 'protein_shake'},
				{label = 'Eau ($1)', value = 'water'},
				{label = 'Déjeuner Healthy ($2)', value = 'sportlunch'},
