...
local Apartments = {
	...
	[2] = {
		label = 'Motel de Luxe',
		enter = {x = 319.333, y = -196.355, z = 53.22},
		inside = {x = 316.961, y = -213.60, z = 38.35},
		exit = {x = 316.768, y = -216.047, z = 38.33},
		closet = {x = 151.8, y = -1001.36, z = -100.0},
		storage = {x = 151.33, y = -1003.08, z = -100.0}
	}
}	
...
Citizen.CreateThread(function()
	...
	CacheApartments(function()
		for k,v in pairs(Apartments) do
			...
			local enterMessage = 'Appuie sur ~INPUT_CONTEXT~ pour entrer dans la chambre d\'hotel'

			if not owns then
				enterMessage = 'Appuie sur ~INPUT_CONTEXT~ pour entrer dans la chambre d\'hotel'
			end
			...
			if owned then
				...
				Markers.AddMarker('apartment_storage', v.storage, 'Appuie sur ~INPUT_CONTEXT~ pour ouvrir le coffre', function()
			...
			if owned or invited then
				...
				Markers.AddMarker('apartment_closet', v.closet, 'Appuie sur ~INPUT_CONTEXT~ pour mettre des vétements', function()
		    ...
		    Markers.AddMarker('apartment_exit', v.exit, 'Appuie sur ~INPUT_CONTEXT~ pour ouvrir le Menu', function()
				local elements = {
					{label = 'Sortir', value = 'exit'}
				}

				if owned or invite then
					table.insert(elements, {label = 'Inviter quelqu\'un', value = 'invite'})
				end
...
function OpenStorageMainMenu(apartment)
	local elements = {
		{label = 'Objets', value = 'items'},
		{label = 'Argent Sale', value = 'safe'},
		{label = 'Droges', value = 'drugs'}
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'apartment_storage_option',
		{
			...
			align = 'top-left',
			elements = {
				{label = 'Prendre', value = 'get'},
				{label = 'Mettre', value = 'put'}
			}
		},
		function(data, menu)
			menu.close()
							...
							if v ~= nil and v.count ~= nil then
								if v.count > 0 then
									if v.money == true then
										table.insert(elements, {label = 'Argent Sale (' .. v.count .. ' $)', value = k, amount = v.count, rawLabel = 'Dirty Money'})								
									else
				        			...
				        			if items[storage] ~= nil and items[storage][itemName] ~= nil then	    
				        				if items[storage][itemName].weapon ~= true then
					        				if items[storage][itemName].count >= count then
							        			...
							        			ESX.ShowNotification('Il n\'y a pas de ~r~x' .. count .. ' ' .. data2.current.rawLabel)
							        		end
							        	end
				        			else
				        				ESX.ShowNotification('Il n\'y a pas de ~r~x' .. count .. ' ' .. data2.current.rawLabel)
					...
					ESX.TriggerServerCallback('esx_sommen_motel:getInventory', function(inventory)
				    	ESX.TriggerServerCallback('esx_sommen_motel:getDirtyMoney', function(money)
					    	...
					    	if storage == 'safe' then
					    		table.insert(elements, {label = 'Argent sale (' .. money .. ' $)', value = 'dirty', amount = money, money = true})	
					    	else
					        		...
					        		if data2.current.money == true then
					        			OpenQuantityMenu(function(count)
					        				...
					        				if money >= count then
					        					...
					        					ESX.ShowNotification("Vous n'avez ~r~pas~w~ de ~r~" .. count .. " $ ~w~d'argent sale.")
					        				end
					        			end)
					        		elseif data2.current.weapon ~= true then
						        		OpenQuantityMenu(function(count)
						        			if count <= 0 then
						        				ESX.ShowNotification('~r~Ogiltigt belopp. Behöver vara större än 0.')
						        			...
						        			ESX.TriggerServerCallback('esx_sommen_motel:hasItem', function(has)
						        				if has == true then
						        					...
						        					ESX.ShowNotification("Vous ne possédez ~r~pas ~w~cette ~w~somme")
						        				end
						        			end, itemName, count)
...
function OpenApartmentMenu(apartment, owned)
	local values = GetApartmentValues(apartment)
		...
		if owned then
			...
			Markers.AddMarker('apartment_' .. apartment, values.enter, 'Appuie sur ~INPUT_CONTEXT~ pour entrer dans la chambre d\'Hotel', function()
				OpenApartmentMenu(apartment, true)
			end)	

			SetEntityCoords(GetPlayerPed(-1), 151.38, -1007.95, -99.0 - 1.0)

			Notifications.PlaySpecialNotification("Bienvenue dans l'hotel")				
    ...
    while true do
        ...
        Citizen.Wait(5)
            ...
            if distance < 3 and InAction == false then
                Drawing.draw3DText(154.2, -1006.01, -101.0, 'Appuie sur ~g~[E] ~w~pour se coucher', 6, 0.07, 0.07, 255, 255, 255, 215)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if InAction == true then
                headsUp('Appuie sur ~INPUT_VEH_DUCK~ pour vous mettre en place')
    ...
    while true do
        ...
        Citizen.Wait(5)
            ...
            if distance < 2.3 and InAction == false then
                Drawing.draw3DText(154.64, -1001.16, -101.0, 'Appuie sur ~g~[E] ~w~pour aller au toilettes', 6, 0.07, 0.07, 255, 255, 255, 215)
              if distance < 1 and InAction == false then
                if IsControlJustReleased(0, Keys['E']) then
