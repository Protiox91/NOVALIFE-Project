...
AddEventHandler('esx:onPlayerDeath', function(data)
	...
	if (bone == 31086) then
		multi = 0.0
		print('HEADSHOT')
		TriggerEvent('chatMessage', "MedSystem", {255, 0, 0}, "Tu as été bléssé au niveau de la tête")
		bleeding = 5
		area = "HEAD"
	end
	if bone == 24817 or bone == 24818 or bone == 10706 or bone == 24816 or bone == 11816 then
		multi = 1.0
		print('BODYSHOT')
		TriggerEvent('chatMessage', "MedSystem", {255, 0, 0}, "Tu as été bléssé au niveau du corps")
		bleeding = 2
		area = "BODY"
	end
...
AddEventHandler('medSystem:near', function(x,y,z, pulse, blood, nameF, nameL, area, bldn)
	...
	if GetDistanceBetweenCoords(x,y,z,a,b,c,false) < 10 then
		timer = 10
		TriggerEvent('chatMessage', "MedSystem", {255,0 , 0}, "^1 [0-5] MORT ^6[5-15] Besoin d'ambulance ^5[15-38] Intervention EMS ^4[38-55] Intervention police ^2[55+] En forme")
		cBlood = blood
		cPulse = pulse
		cNameF = nameF
		cNameL = nameL
