...
RegisterNetEvent('BrookAI:callTaxi')
AddEventHandler('BrookAI:callTaxi', function(coords)
	if customer then
		ESX.ShowHelpNotification('Un Taxi est déjà en route')
	else
		customer = coords
		-- get best spawnpoint
...
RegisterNetEvent('BrookAI:setTaxiBlip')
AddEventHandler('BrookAI:setTaxiBlip', function(coords)
	if CarBlip then
		RemoveBlip(CarBlip)
		CarBlip = nil
...
RegisterNetEvent('BrookAI:killTaxiBlip')
AddEventHandler('BrookAI:killTaxiBlip', function()
	RemoveBlip(CarBlip)
end)

RegisterNetEvent('BrookAI:cancelTaxi')
AddEventHandler('BrookAI:cancelTaxi', function(message)
	atTarget(message)
end)
		...
		if customer ~= nil then
			...
			if vehicle == globalTaxi then
				...
				if not DoesBlipExist(waypoint) and not onTour then
					ESX.ShowHelpNotification('Où voulez-vous aller ?')
					Citizen.Wait(2000)
				else
			...
			if customer ~= nil then
				...
				if distanceMeTaxi <= 40 then
					if not parkingDone then
						...
						TriggerEvent('esx:showNotification', 'Votre taxi est arrivé !')
					end
	...
	while true do
		...
		if customer ~= nil then
			if taxiArrived and not inCar and not onWayBack then
				ESX.ShowHelpNotification('Appuie sur ~INPUT_PICKUP~ pour entrer')
...
Citizen.CreateThread(function()
	while true do
		...
		if taxiBlip then
			...
			TriggerEvent('BrookAI:setTaxiBlip', coords)
		end
	end
end)
...
function atTarget(cancel)
	...
	if cancel then
		...
		local vehicle = GetVehiclePedIsIn(playerPed, false)
		if vehicle ~= globalTaxi then
			TriggerEvent('esx:showNotification', 'La course a été annulée')
			cancelTaxi = true
		else
			TriggerEvent('esx:showNotification', 'La course ne peut pas être annulée')
			return
		end
	end
	if not cancelTaxi then
		ESX.ShowHelpNotification('Nous sommes arrivés à la destination')
		...
		TriggerServerEvent('BrookAI:pay', price)
...
function drive(x, y , z, delete, status)
	if status == 'start' then
		...
		ESX.ShowHelpNotification('Un chauffeur est en route pour vous.')
	elseif status == 'end' then
		ESX.ShowHelpNotification('Merci pour votre confiance')
	end
