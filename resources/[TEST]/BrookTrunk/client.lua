	...
	Citizen.CreateThread(function()
	  	while true do
		    ...
		    if IsDisabledControlPressed(0, 182) and GetVehiclePedIsIn(player, false) == 0 and DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) then
		    	...
		    	if not inside then
		        	AttachEntityToEntity(player, vehicle, -1, 0.0, -2.2, 0.5, 0.0, 0.0, 0.0, false, false, false, false, 20, true)		       		
		       		...
		       		if IsEntityAttached(player) then
						...
						AddTextComponentString('~s~~INPUT_CELLPHONE_CAMERA_FOCUS_LOCK~ pour sauter du coffre')
		    	...
		    	elseif inside and IsDisabledControlPressed(0, 182) then
	    	...
	    	if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and not inside and GetVehiclePedIsIn(player, false) == 0 then
				SetTextComponentFormat("STRING")
				AddTextComponentString('~s~~INPUT_CELLPHONE_CAMERA_FOCUS_LOCK~ pour rentrer dans le coffre')
