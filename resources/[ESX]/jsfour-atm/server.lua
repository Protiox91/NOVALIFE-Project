...
AddEventHandler('jsfour-atm:insert', function(amount)
	...
	if amount > xPlayer.getMoney() then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Vous déposez ' .. amount .. '~s~ $')
	end
end)
...
AddEventHandler('jsfour-atm:take', function(amount)
	...
	if amount > accountMoney then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Vous retirez ' .. amount .. '~s~ $')
	end
end)
  ...
  function (result)
    if (result[1] ~= nil) then
      ...
      local senPlayer    = ESX.GetPlayerFromId(_source)
        ...
        function (result)
          if (result[1] ~= nil) then
            TriggerClientEvent('esx:showNotification', _source, 'Vous avez envoyé ' .. amount .. '~s~ $ a ' .. result[1].firstname .. ' ' .. result[1].lastname)
            TriggerClientEvent('esx:showNotification', recPlayer.source, 'Tu as reçus ' .. amount .. '~s~ $ de la part de ' .. result[1].firstname .. ' ' .. result[1].lastname)
          end
        end)
      end
    end
