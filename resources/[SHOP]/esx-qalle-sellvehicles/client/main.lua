				...
				if dstCheck <= 4.2 then
					...
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							...
							ESX.ShowNotification("Vous devez être dans le ~g~véhicule !")
						end
					end
				end
			end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			...
			table.insert(elements, { ["label"] = "Enlever de la vente", ["value"] = "sell" })
		else
			table.insert(elements, { ["label"] = "Mettre le prix - :-", ["value"] = "price" })
		end
	else
		table.insert(elements, { ["label"] = "Acheter " .. price .. " - :-", ["value"] = "buy" })

		if owner then
			table.insert(elements, { ["label"] = "Enlever le véhicule de la vente", ["value"] = "remove" })
		end
	end
