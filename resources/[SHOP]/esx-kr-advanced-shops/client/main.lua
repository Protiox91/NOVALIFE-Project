...
function OpenShopCenter()
	...
	ESX.UI.Menu.CloseAll()
  	...
  	local elements = {}
		...
		if showblip then
			table.insert(elements, {label = 'Cacher les boutiques dispo', value = 'removeblip'})
		else
			table.insert(elements, {label = 'Montrer les boutiques dispo', value = 'showblip'})
		end
...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Vous avez ' .. data[1].money .. '$ dans votre coffre',    value = ''})
		table.insert(elements, {label = 'Livraisons',    value = 'shipments'})
        table.insert(elements, {label = 'Mettre en vente un objet', value = 'putitem'})
        table.insert(elements, {label = 'Enlever de la vente un objet',    value = 'takeitem'})
        table.insert(elements, {label = 'Mettre de l\'argent dans le coffre',    value = 'putmoney'})
        table.insert(elements, {label = 'Retirer l\'argent du coffre',    value = 'takemoney'})
        table.insert(elements, {label = 'Changer le nom de la boutique : $' .. Config.ChangeNamePrice,    value = 'changename'})
		table.insert(elements, {label = 'Vendre la boutique pour $' .. math.floor(data[1].ShopValue / Config.SellValue),   value = 'sell'})
        ...
        elseif data.current.value == 'takemoney' then
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'takeoutmoney', {
                title = 'Combien d\'argent voulez-vous récupérer ?'
            }, function(data2, menu2)
		...
		menu2.close()
	 	...
	 	elseif data.current.value == 'putmoney' then
			...
			title = 'Combien d\'argent voulez-vous mettre dans le coffre ?'
		...
		elseif data.current.value == 'sell' then
		  ...
		  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell', {
			title = 'Ecrit: (YES) sans les parenthèses pour valider'
	  ...
	  elseif data.current.value == 'changename' then
		...
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'changename', {
		  title = 'Comment voulez-vous que votre boutique se nomme ?'
...
function OpenShipments(id)
	...
	table.insert(elements, {label = 'Commander Produits', value = 'buy'})
	table.insert(elements, {label = 'Livraisons', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = '-- COLIS --'})
			table.insert(elements, {label = 'Récupérez vos colis', value = 'pickup'})
			once2 = false
			end
			...
			if time - items[i].time <= Config.DeliveryTime and once then
				table.insert(elements, {label = '-- LIVRAISONS --'})
				once = false
			end

			if time - items[i].time <= Config.DeliveryTime then
				times = time - items[i].time
				table.insert(elements, {label = items[i].label .. ' temps restant : ' .. math.floor((Config.DeliveryTime - times) / 60) .. ' mn' })
			end
		...
		ESX.TriggerServerCallback('esx_kr_shop-robbery:getOnlinePolices', function(results)
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
					...
					ESX.ShowNotification("~r~Il n'y a pas assez de policier en ville : " .. results .. '/' .. Config.RequiredPolices)
				end
			else
				ESX.ShowNotification("~r~Cette boutique a déjà été braquée précedemment.")
			end
		end)
	end, id)
end
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 15 then
					...
					TriggerEvent("mt:missiontext", "Essaie d' ouvrir le coffre :~r~ " .. hp/100 .. "%", 1000)

				if hp == 0 then
					OnRobbery = false
					TriggerServerEvent('esx_kr_shops-robbery:GetReward', Id)
					TriggerServerEvent("esx_kr_shops-robbery:NotifyOwner", '~r~Le braquage de votre boutique ~b~(' .. Name ..')~r~ a malheureusement été un succès.', Id)
					DeleteEntity(Var)
				end
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					OnRobbery = false
					DeleteEntity(Var)
					TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~g~Le braquage de votre boutique ~b~(" .. Name .. ')~g~ a heureusement été raté.', Id)
