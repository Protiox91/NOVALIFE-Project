      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Vous vendez déjà cet objet, ~r~mais pour ' .. data[1].price .. '$. Vous avez mis ' .. Price'$')
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Enlevez-le des rayons et remttez-le à un différent prix !')
            end
        end)  
    end)
end)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Vous avez commandé' .. count .. ' pièces ' .. label .. ' pour ' .. price * count'$')
        else
            TriggerClientEvent('esx:showNotification', _source, '~r~Vous navez pas assez d\'argent dans votre shop.')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~gVous avez acheté une boutique pour ' ..  price'$')
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~Vous ne pouvez pas l\'acheter.')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Les bénéfices de votre boutique ont été gelées, attendez ' .. math.floor((900 - time) / 60) .. ' minutes')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Vous avez mis ' .. amount .. '$ dans votre boutique.')
        else
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Vous ne pouvez pas vendre plus de ce que vous possédez !')
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Vous avez pris ' .. amount .. '$ de votre shop')
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Vous avez vendus votre shop')
            else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Vous ne pouvez pas vendez votre shop avec des objets et de l\'argent dedans')
            end
        end)
    end)
end)
