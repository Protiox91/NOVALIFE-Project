...
$(function() {
		window.addEventListener('message', function(event) {
			...
			for (i = 0; i < event.data.result.length; i++) {
				...
				$('#wrapper').append(
					`<div class = "image" id = ${event.data.result[i].item} label = ${event.data.result[i].label} count = ${event.data.result[i].count} price = ${event.data.result[i].price}>
						...
						<h5 class = "h4">Stock : ${event.data.result[i].count}x</h5>
					</div>`
				);
			}

			$('#wrapper').append(
				` <h6 class = "h4" style = "right: 47.125%; position: absolute;" bottom = ${id - 5}>Rework by Jack Brook</h6>`
			)
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">Gestion SHOP</button>
					`
				);
			}
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "Combien ?"></textarea>
							</div>`
								);
							};
							...
							$('#cart').append(
							`
							<button class = "button" id = "buybutton" style = "position: absolute; right: 15px; top: 5px;">Acheter</button>
							<button class = "button" id = "back" style = "position: absolute; left: 15px; top: 5px;">Retour</button>
							`
						);
					});
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$.post('http://esx-kr-advanced-shops/notify', JSON.stringify({msg : "~r~Il n'y a pas assez de stocks !"}));
					}
				}
			});
