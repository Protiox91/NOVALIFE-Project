	...
	local source = source
		...
		if PlayersWashing[source] == true then
			...
			if blackMoney.money < Config.Slice then
				TriggerClientEvent("esx_moneywash:notify", source, "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('Nocash') .. Config.Slice)
			else
				...
				TriggerClientEvent("esx_moneywash:notify", source, "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('cash') .. washedMoney .. _U('cash1'))
			end
		end
	end)
end
...
AddEventHandler('esx_moneywash:washMoney', function(amount)
	...
	if amount > 0 and account.money >= amount then
		...
		TriggerClientEvent("esx_moneywash:notify", source, "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('invalid_amount'))
	end
